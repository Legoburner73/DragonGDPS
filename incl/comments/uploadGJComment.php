...
if($id != "" AND $comment != ""){
	...
	if($register == 1){
		...
		$query->execute([':userName' => $userName, ':comment' => $comment, ':levelID' => $levelID, ':userID' => $userID, ':uploadDate' => $uploadDate, ':percent' => $percent]);
		$inside_data = array('username' => $userName, 'message' => $decodecomment, 'levelID' => $levelID, 'userID' => $userID, 'uploadDate' => $uploadDate);
            	$inside_data_string = json_encode($inside_data);
            	$data = array("event" => "comment_upload", "data" => $inside_data_string);                                                                    
